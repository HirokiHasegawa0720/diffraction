...
bb=df['Tvn[K]']
cc=df['M_CJkg/kmol']
d = {'pc[bar]':aa ,'Tvn[K]':bb ,'M_CJkg/kmol':cc,'反射点距離':output}
dfs = pd.DataFrame(d)
...
#sol=['adam','sgd']
sol=['adam']
#act=['relu','tanh','logistic','identity']
act=['relu']
...
test_score=grid.score(x_tests, y_tests)
y_preds=grid.predict(x_tests)
print(mean_squared_error(y_preds, y_tests))
